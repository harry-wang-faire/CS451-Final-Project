...
# # split the covid data into multiple rdds based on geo_code
# train_df = train_df.groupBy('geo_code').agg(\
#   collect_list('date').alias('date'),\
#   collect_list('numtested').alias('numtested'),\
#   collect_list('numconfirmed').alias("numconfirmed"),\
# ).withColumn("date", zip("numtested", "numconfirmed")).drop("numtested", "numconfirmed")

# test_df = test_df.groupBy('geo_code').agg(\
#   collect_list('date').alias('date'),\
#   collect_list('numtested').alias('numtested'),\
#   collect_list('numconfirmed').alias("numconfirmed"),\
# ).withColumn("date", zip("numtested", "numconfirmed")).drop("numtested", "numconfirmed")
